...
def create_model(args):
    ...
    model = Sequential(
        ...
        NMSuppression(), 
        DoubleThreshold(args.th_low, args.th_high)
    )
    return model

def edge_opencv(args, img_path):
    """
    生成使用opencv进行边缘检测的图像
    """
    img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
    img = cv2.Canny(img, args.th_low, args.th_high)
    return img
...
def main(args):
    ...
    for img_path in img_paths:
        ...
        if args.compare:
            output_name = f"{original_filename}-{args.operator}-{args.th_low}-{args.th_high}-compare.{args.output_type}"
            img_cv = edge_opencv(args, img_path)
            img.save(args.output_dir, output_name, compare=img_cv)
        else:
            output_name = f"{original_filename}-{args.operator}-{args.th_low}-{args.th_high}.{args.output_type}"
